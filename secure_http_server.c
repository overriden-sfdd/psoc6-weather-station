...
/* i2c_sensor and BMP280 header file */ /* <NEW> */
#include "i2c_utils.h"                  /* <NEW> */
#include "bmp2.h"                       /* <NEW> */
...
/* BMP280 sensor data. */
static struct bmp2_data comp_data;
...
/******************************************************************************
 * Function create web page
 *******************************************************************************/
void create_web_page(char *page_buf)
{
	char *bg_color[] = { "black", "red" };
	int bg_index = 0;
	int i;

	get_bmp280_data(&comp_data);

	if (strcmp(led_status, LED_STATUS_ON) == 0) {
		bg_index = 1 ;
	}
	for (i = 0 ; i < MAX_HTTP_RESPONSE_LENGTH ; i++ ) {
		page_buf[i] = 0 ;
	}

	sprintf(page_buf, HTTPS_STARTUP_WEBPAGE,
			HTTPS_REFRESH_INTERVAL,
			ntohl(ip_addr.ip.v4),
			HTTPS_PORT,
		...
		cy_http_message_body_t* https_message_body)
{
    ...
    create_web_page(https_response);                                    /* <NEW> */
